...
def do_start(state):
    ...
    for k in range(0, 128):
        o = state.solver.BVS("input_string_" + str(k), 8)
        state.solver.add(state.solver.Or(state.solver.And(o >= 60, o <= 127), o == 0))

        if arg == None:
            arg = o
        else:
            arg = state.solver.Concat(arg, o)
...
def do_end(state, params, pg, verbose=True):
    if verbose:
        print(state.solver.eval(params['arg']))
    ...
    for b in params['arg'].chop(8):
            s = state.solver.eval_upto(b, 256)
